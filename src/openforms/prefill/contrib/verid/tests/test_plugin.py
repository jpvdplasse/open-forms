...
from ..plugin import VerIDPrefill
...
class ConfigurationTests(SimpleTestCase):
    ...
    def test_configuration_context(self):
        configuration_context = VerIDPrefill.configuration_context()
    ...
    def test_get_available_attributes_returns_oidc_claim_suggestions(self):
        # The form-builder UI uses a free-text input for Ver.iD now (see
        # patches/@open-formulieren+formio-builder+0.51.0.patch), but other
        # tooling (validation, schema export) may still call this method —
        # surface the standard OIDC claim names Ver.iD advertises.
        attributes = list(VerIDPrefill.get_available_attributes())

        self.assertTrue(all(len(t) == 2 for t in attributes))
        names = {key for key, _label in attributes}
        # Spot-check a few of the OIDC claim names we expose.
        self.assertIn("given_name", names)
        self.assertIn("family_name", names)
        self.assertIn("address.postal_code", names)


class VerIDPluginTests(TestCase):
    maxDiff = None

    def test_populate_fixed_attribute_identifier(self):
        plugin = VerIDPrefill("verid")
    ...
    def test_only_support_main_identifier_role(self):
        """
        VerID authentication/prefill does not support mandates.
        """
        plugin = VerIDPrefill("verid")
